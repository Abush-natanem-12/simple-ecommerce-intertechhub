...
import { getProductsByCategory } from "../hooks/requests";
  ...
  useEffect(
    function () {
      const fetchProducts = async function () {
        try {
          setLoading(true);
          const limitedProducts = await getProductsByCategory(category);
  ...
  return (
    <div className="w-[95%] md:w-[90%] xl:w-[85%] 2xl:w-[80%] mx-auto mb-14 lg:mb-24 flex flex-col gap-5">
      <div className="flex items-center gap-2">
        <span className="py-4 px-2 bg-button2"></span>
        <h4 className="text-xs lg:text-base text-button2">available</h4>
      </div>

      <div className="w-full flex gap-10 items-end">
        <h2 className="text-2xl md:text-3xl 2xl:text-4xl tracking-[1px] font-[500] uppercase">
          {category}
        </h2>
